    ...
    with torch.no_grad():
        ...
        with tqdm(test_loader1, mininterval=5.0, maxinterval=50.0) as it:
            for batch_no, test_batch in enumerate(it, start=1):
                ...
                trend = test_batch["trend"].unsqueeze(1)  
                trend = trend.to(samples.device)  
    ...
    with torch.no_grad():
        ...
        with tqdm(test_loader1, mininterval=5.0, maxinterval=50.0) as it:
            for batch_no, test_batch in enumerate(it, start=1):
                ...
                trend2 = test_batch2["trend"]  
                trend2 = trend2.to(samples2.device)  
                ...
                # 只对重建结果加回趋势
                samples_with_trend = samples_median + trend  
