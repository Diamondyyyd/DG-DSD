...
def getData(path='./dataset/', dataset='SWaT', period=1440, train_rate=0.8):
    # 直接从path加载文件，不再添加dataset子目录
    init_data = np.load(path + dataset + '_train_data.npy')
    init_time = getTimeEmbedding(np.load(path + dataset + '_train_date.npy'))

    test_data = np.load(path + dataset + '_test_data.npy')
    test_time = getTimeEmbedding(np.load(path + dataset + '_test_date.npy'))
    test_label = np.load(path + dataset + '_test_label.npy')
