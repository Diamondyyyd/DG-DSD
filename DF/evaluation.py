...
def merge(pkl_path,data_id,machine_number = ""):
    ...
    if data_id == "GCP":
        ...
        labels = np.load(
            f"data/Machine/{machine_number}_test_label.npy"
        )
        ground_truth = np.load(
            f"data/Machine/{machine_number}_test_data.npy"
        )
    else:
        labels = np.load(
            f"data/Machine/{data_id}_test_label.npy"
        )
        ground_truth = np.load(
            f"data/Machine/{data_id}_test_data.npy"
        )
    ...
    return all_gen_middle, labels, all_target
...
def ensemble(pkl_path, data_id, ensemble_strategy_list = [],last_step_threshold = 0.02,compute_abs=True,compute_sum=True,machine_number=""):
    ...
    if data_id == "GCP":
        all_gen_middle, labels, all_target = merge(pkl_path, data_id,machine_number=machine_number)
    else:
        all_gen_middle, labels, all_target = merge(pkl_path,data_id)
    residual_list = []
    ...
    for ensemble_threshold in range(0,len(residual_list)):
        ...
        adjust_prediction = prediction_adjust(prediction,labels)
        p,r,f = compute_f(adjust_prediction,labels)
